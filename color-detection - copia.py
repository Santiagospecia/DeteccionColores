...
cap.set(3, 1000)
cap.set (4, 3000) # Establezca la resolución de la cámara, 3 es alta, 4 es ancha
...
while(1):
    _, img = cap.read()
    rec = cv2.rectangle(img, start_point, end_point, color, thickness)
    ...
    for pic, contour in enumerate(contours):
        ...
        if(area > 300):
            ...
            if (x >= 200 and y >= 100 and (x+w) <= 800 and (y+h) <= 300):
